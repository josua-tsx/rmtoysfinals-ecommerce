...
export default function WishlistCard({ productWish }) {
  ...
  const {mutate: addWishToCartMutation, isPending, isError} = useMutation({
    mutationFn: async (productId) => {
        const res = await axiosInstance.put(`/cart/addWishToCart`, productId)
        return res.data 
    }, 
    onSuccess: () => {
        queryClient.invalidateQueries({queryKey: ['wishlist']})
        toast.success("Successfully transfered to cart!")
    },
    onError: (err) => {
  ...
  const handleWishToCart = () => {
    addWishToCartMutation({ productId: productWish.productId._id });
  }
  ...
  return (
    <div className="border w-[253px] mx-auto md:w-full border-black bg-card relative flex justify-center items-center flex-col rounded-[5px]">
      <button type="button" className="absolute right-0 text-red-700 top-0">
        <MdDelete size={25} />
      </button>
      ...
      <button onClick={handleWishToCart}
