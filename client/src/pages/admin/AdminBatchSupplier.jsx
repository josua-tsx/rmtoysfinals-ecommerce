...
export default function AdminBatchSupplier() {
  ...
  const { mutate: uploadMutation, isPending: isUploading } = useMutation({
    mutationFn: async (formData) => {
      const res = await axiosInstance.post(`/supplier/batch-add`, formData, {
    ...
    onSuccess: (data) => {
      ...
      setFile(null);
      if (data.created > 0) {
        toast.success(`Successfully created ${data.created} suppliers!`);
      }
      if (data.failed > 0) {
  ...
  const handleDownloadTemplate = async () => {
    try {
      const response = await axiosInstance.get(`/supplier/csv-template`, {
        responseType: "blob",
      });
  ...
  return (
    <section className="bg-yellow min-h-screen text-sm md:text-normal font-main pb-20">
      <AdminHeader title="BATCH SUPPLIER UPLOAD" />
      ...
      <div className="max-w-[90%] pt-14 pb-5 mx-auto flex gap-6 flex-col">
        ...
        <div className="border border-black rounded-[5px] bg-card p-6">
          ...
          <ol className="list-decimal list-inside space-y-2 text-sm">
            ...
            <li>
              <strong>Fill in details</strong> - Name, Contact Person, Number,
              Address.
            </li>
            <li>
              <strong>Upload CSV</strong> - The system will create valid
              suppliers and skip duplicates.
            </li>
          </ol>
        </div>
