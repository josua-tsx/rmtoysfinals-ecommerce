...
export default function AdminEditGcash() {
  ...
  const {
    ...
    queryKey: ["gcash"],
    queryFn: async () => {
      const { gcashId } = params;
      const res = await axiosInstance.get(`/gcash/get-single/${gcashId}`);
      return res.data;
    },
  });

  console.log(singleGcash);

  useEffect(() => {
    if (singleGcash) {
      setGcashImage(singleGcash?.gcashUrl);
      setGcashName(singleGcash?.gcashName);
    }
  }, [singleGcash]);

  const { mutate: updateGcashMutation } = useMutation({
    mutationFn: async (data) => {
      const res = await axiosInstance.put(
        `/gcash/edit-gcash/${singleGcash._id}`,
        data
      );
      return res.data;
    },
