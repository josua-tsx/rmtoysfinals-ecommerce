...
export default function AdminBatchFaq() {
  ...
  const { mutate: uploadMutation, isPending: isUploading } = useMutation({
    mutationFn: async (formData) => {
      const res = await axiosInstance.post(`/faqs/batch/upload`, formData, {
    ...
    onSuccess: (data) => {
      ...
      setFile(null);
      if (data.created > 0) {
        toast.success(`Successfully created ${data.created} FAQs!`);
      }
      if (data.failed > 0) {
  ...
  const handleDownloadTemplate = async () => {
    try {
      const response = await axiosInstance.get(`/faqs/batch/template`, {
        responseType: "blob",
      });
  ...
  return (
    <section className="bg-yellow min-h-screen text-sm md:text-normal font-main pb-20">
      <AdminHeader title="BATCH FAQ UPLOAD" />
      ...
      <div className="max-w-[90%] pt-14 pb-5 mx-auto flex gap-6 flex-col">
        ...
        <div className="border border-black rounded-[5px] bg-card p-6">
          ...
          <ol className="list-decimal list-inside space-y-2 text-sm">
            ...
            <li>
              <strong>Fill in details</strong> - Title (Question) and Answer.
            </li>
            <li>
              <strong>Max Limit</strong> - You can only have a total of 5 FAQs.
            </li>
            <li>
              <strong>Upload CSV</strong> - The system will create valid FAQs
              and skip duplicates.
            </li>
          </ol>
        </div>
