...
export default function AdminBatchCategory() {
  ...
  const { mutate: uploadMutation, isPending: isUploading } = useMutation({
    mutationFn: async (formData) => {
      const res = await axiosInstance.post(`/category/batch-add`, formData, {
    ...
    onSuccess: (data) => {
      ...
      setFile(null);
      if (data.created > 0) {
        toast.success(`Successfully created ${data.created} categories!`);
      }
      if (data.failed > 0) {
  ...
  const handleDownloadTemplate = async () => {
    try {
      const response = await axiosInstance.get(`/category/csv-template`, {
        responseType: "blob",
      });
      const url = window.URL.createObjectURL(new Blob([response.data]));
      const link = document.createElement("a");
      link.href = url;
      link.setAttribute("download", "category_upload_template.csv");
  ...
  return (
    <section className="bg-yellow min-h-screen text-sm md:text-normal font-main pb-20">
      <AdminHeader title="BATCH CATEGORY UPLOAD" />
      ...
      <div className="max-w-[90%] pt-14 pb-5 mx-auto flex gap-6 flex-col">
        ...
        <div className="border border-black rounded-[5px] bg-card p-6">
          ...
          <ol className="list-decimal list-inside space-y-2 text-sm">
            ...
            <li>
              <strong>Fill in categories</strong> - Add one category per row.
            </li>
            <li>
              <strong>Upload CSV</strong> - The system will create valid
              categories and skip duplicates.
            </li>
          </ol>
        </div>
