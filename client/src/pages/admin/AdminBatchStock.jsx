...
export default function AdminBatchStock() {
  ...
  const { mutate: uploadMutation, isPending: isUploading } = useMutation({
    mutationFn: async (formData) => {
      const res = await axiosInstance.post(`/stocks/batch-order`, formData, {
    ...
    onSuccess: (data) => {
      ...
      if (data.created > 0) {
        toast.success(
          `Successfully ordered stock for ${data.created} products!`,
        );
      }
      if (data.failed > 0) {
  ...
  const handleDownloadTemplate = async () => {
    try {
      const response = await axiosInstance.get(`/stocks/csv-template`, {
        responseType: "blob",
      });
  ...
  return (
    <section className="bg-yellow min-h-screen text-sm md:text-normal font-main pb-20">
      <AdminHeader title="BATCH STOCK UPLOAD" />
      ...
      <div className="max-w-[90%] pt-14 pb-5 mx-auto flex gap-6 flex-col">
        ...
        <div className="border border-black rounded-[5px] bg-card p-6">
          <h2 className="font-black uppercase tracking-widest mb-4">
            How to Use Batch Stock
          </h2>
          <ol className="list-decimal list-inside space-y-2 text-sm">
            <li>
              <strong>Download the template</strong> - Get the CSV with correct
              headers.
            </li>
            <li>
              <strong>Fill in details</strong> - Match <code>productName</code>{" "}
              exactly to your existing product.
            </li>
            <li>
              <strong>Upload CSV</strong> - The system will create stock entries
              and publish valid products.
            </li>
          </ol>
          ...
          <div className="mt-6 p-4 bg-amber-50 border border-amber-500 rounded-[5px]">
            ...
            <ul className="text-xs space-y-1 text-amber-800">
              <li>
                • <strong>productName</strong> must match EXACTLY
                (case-insensitive).
              </li>
              <li>
                • <strong>supplierName</strong> must match an existing supplier.
              </li>
              <li>
                • <strong>dateDelivery</strong> format: YYYY-MM-DD.
              </li>
              <li>
                • This will automatically set product status to{" "}
